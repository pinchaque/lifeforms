class Simulation
  ...
  # Adds a lifeform to the specified location.
  def add_lifeform(l, loc)
    ...
    LifeformLoc.new(environment_id: id, lifeform_id: l.id, x: loc.x, y: loc.y).save    
  end

  # Adds a lifeform to the environment at a random location within it.
  def add_lifeform_rnd(l)
    add_lifeform(l, Location.random(width, height))
  end
  ...
  def add_lifeform_dist(l, loc, dist)
    add_lifeform(l, Location.at_dist(width, height, loc, dist))
  end
